...
class TModel:
    def __init__(self, args, model_type,w=16):
        ...
        self.weights = {
            ...
            'wc1': tf.Variable(tf.random_normal([5, 5, 1, 2*w]), name="%s_%s" % (self.model_type, "wc1")),
            # 5x5 conv, 32 inputs, 64 outputs
            'wc2': tf.Variable(tf.random_normal([5, 5, 2*w, 4*w]), name="%s_%s" % (self.model_type, "wc2")),
            # fully connected, 7*7*64 inputs, 1024 outputs
            'wd1': tf.Variable(tf.random_normal([7 * 7 * 4*w, 64*w]), name="%s_%s" % (self.model_type, "wd1")),
            # 1024 inputs, 10 outputs (class prediction)
            'out': tf.Variable(tf.random_normal([64*w, self.num_classes]), name="%s_%s" % (self.model_type, "out"))
        }
    ...
    def perturb(self, x_nat, y):
        ...
        for i in range(40):
            grad_ = self.sess.run(self.grad, feed_dict={self.X: x_, self.Y: y, self.keep_prob: self.dropoutprob,
    ...
    def train(self, dataset):
        ...
        for step in range(1, self.num_steps + 1):
            ...
            x_perturb = self.perturb(batch_x, batch_y)
            '''
            ...
            '''
            _, summary = self.sess.run([self.train_op, self.merged_summary_op],
                                       feed_dict={self.X: x_perturb, self.Y: batch_y, self.keep_prob: self.dropoutprob,
                                                  self.softmax_temperature: self.temperature})
            if (step % self.display_step) == 0 or step == 1:
                # Calculate Validation loss and accuracy
                validation_x, validation_y = dataset.get_validation_data()
                x_val_perturb = self.perturb(validation_x, validation_y)
                loss, acc = self.sess.run([self.loss_op, self.accuracy], feed_dict={self.X: validation_x,
                                                                                    self.Y: validation_y,
                                                                                    self.keep_prob: 1.0,
                                                                                    self.softmax_temperature: 1.0})
                
                adv_loss, adv_acc = self.sess.run([self.loss_op, self.accuracy], feed_dict={self.X: x_val_perturb,
                                                                                    self.Y: validation_y,
                                                                                    self.keep_prob: 1.0,
                                                                                    self.softmax_temperature: 1.0})
                ...
                print("Step " + str(step) + ", Validation Nat Loss= " + "{:.4f}".format(
                    loss) + ", Validation Nat Accuracy= " + "{:.3f}".format(acc))
                print("Step " + str(step) + ", Validation Adv Loss= " + "{:.4f}".format(
                    adv_loss) + ", Validation Adv Accuracy= " + "{:.3f}".format(adv_acc))
        else:
    ...
    def run_inference(self, dataset):
        ...
        x_test_perturb = self.perturb(test_images, test_labels)
        print("Testing Nat Accuracy:", self.sess.run(self.accuracy, feed_dict={self.X: test_images,
                                                                           self.Y: test_labels,
                                                                           self.keep_prob: 1.0,
                                                                           self.softmax_temperature: 1.0
                                                                           }))
        
        print("Testing Adv Accuracy:", self.sess.run(self.accuracy, feed_dict={self.X: x_test_perturb,
                                                                           self.Y: test_labels,
                                                                           self.keep_prob: 1.0,
                                                                           self.softmax_temperature: 1.0
                                                                           }))
...
class SModel:
    def __init__(self, args, model_type,w=8):
        self.w = w
        self.learning_rate = 0.001
        self.num_steps = args.num_steps
        self.batch_size = args.batch_size
        self.display_step = args.display_step
        self.n_hidden_1 = 5*5*w  # 1st layer number of neurons
        self.n_hidden_2 = 5*5*w  # 2nd layer number of neurons
        self.num_input = 784  # MNIST data input (img shape: 28*28)
        self.num_classes = 10
        self.temperature = args.temperature
        self.checkpoint_dir = args.checkpoint_dir
    ...
    def build_model(self):
        ...
        with tf.name_scope("%soptimization" % (self.model_type)), tf.variable_scope(
            ...
            self.grad = tf.gradients(self.total_loss, self.X)[0]
    ...
    def perturb(self, x_nat, y):
        ...
        #   grad = tf.gradients(loss, x)[0]
        for i in range(40):
            grad_ = self.sess.run(self.grad, feed_dict={self.X: x_, self.Y: y, self.soft_Y: y,
                                                  self.flag: False, self.softmax_temperature: self.temperature})
    ...
    def train(self, dataset, teacher_model=None):
        ...
        def dev_step():
            ...
            loss, acc = self.sess.run([self.loss_op_standard, self.accuracy], feed_dict={self.X: validation_x,
                                                                                         ...
                                                                                         self.flag: False,
                                                                                         self.softmax_temperature: 1.0})
            adv_loss, adv_acc = self.sess.run([self.loss_op_standard, self.accuracy], feed_dict={self.X: x_val_perturb,
                                                                                    self.Y: validation_y,
                                                                                    self.flag: False,
                                                                                    self.softmax_temperature: 1.0})
            if acc > max_accuracy:
